...
parser.add_argument( '--system' ,help='Tracing system used' ,
                     choices=['cadets' ,'clearscope' ,'fivedirections' ,'theia' ,'trace'] ,required=True )
parser.add_argument( '--format' ,help='Consume Avro/JSON serialised CDM (Only JSON is supported as of 01-04-19)' ,
                     default='json' ,
...
def process_cdm_event(record_value ,input_format):
    ...
    if type_value == 'EVENT_CLOSE':
        ...
        if type( object1 ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_ADD_OBJECT_ATTRIBUTE: object does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            object1UUID = object1[CDM_UUID]
            values['srcUUID'] = object1UUID
        ...
        if type( object2 ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_ADD_OBJECT_ATTRIBUTE: object2 does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
        ...
        if type( subject ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_MODIFY_FILE_ATTRIBUTES: subject does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            subjectUUID = subject[CDM_UUID]
            values['srcUUID'] = subjectUUID
        ...
        if type( object1 ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_MODIFY_FILE_ATTRIBUTES: object does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            object1UUID = object1[CDM_UUID]
            values['dstUUID'] = object1UUID
    ...
    # clearscope
        ...
        if type( subject ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_CHECK_FILE_ATTRIBUTES: subject does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            subjectUUID = subject[CDM_UUID]
            values['dstUUID'] = subjectUUID
        ...
        if type( object1 ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_CHECK_FILE_ATTRIBUTES: object does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            object1UUID = object1[CDM_UUID]
            values['srcUUID'] = object1UUID
        ...
        if type( subject ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_WRITE_SOCKET_PARAMS: subject does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            subjectUUID = subject[CDM_UUID]
            values['srcUUID'] = subjectUUID
        ...
        if type( object1 ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_WRITE_SOCKET_PARAMS: object does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            object1UUID = object1[CDM_UUID]
            values['dstUUID'] = object1UUID
        ...
        if type( subject ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_READ_SOCKET_PARAMS: subject does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            subjectUUID = subject[CDM_UUID]
            values['dstUUID'] = subjectUUID
        ...
        if type( object1 ).__name__ == 'NoneType':
            logging.debug(
                "EVENT_READ_SOCKET_PARAMS: object does not exist. Event ID: " + repr( record_value['uuid'] ) )
        else:
            object1UUID = object1[CDM_UUID]
            values['srcUUID'] = object1UUID
    ...
    for line in fh:
        ...
        if cdm_record_type == CDM_TYPE_EVENT:
            ...
            if values['srcUUID'] not in nodes:
                ...
                logging.debug( 'An unmatched source UUID from edge (' + repr( values['uuid'] ) + ') of type: ' + values[
                    'type'] + '.' )
                continue
            else:
            ...
            if values['dstUUID'] not in nodes:
                ...
                logging.debug(
                    'An unmatched destination UUID from edge (' + repr( values['uuid'] ) + ') of type: ' + values[
                        'type'] + '.' )
                continue
            else:
            ...
            # print( str( cdm_record_value['subject'] ), "------subject" )
            # print(str( cdm_record_value['type'] ))
            # print(str( cdm_record_value['predicateObjectPath'] ), "-------object")
            ...
            #         srcNode['nid'] ) + '\t' + dstNodeLabel + ":" + srcNodeLabel + ":" + labelgen( values ) + ":" + str(
            #         values['timestamp'] ) + "\t" + "\n" )
...
if system == 'cadets':
    ...
    second = ['cadets-e3-1-0.json' ,'cadets-e3-1-1.json' ,'cadets-e3-1-2.json' ,'cadets-e3-1-3.json' ,
              'cadets-e3-1-4.json']
    ...
    first = ['theia-e3-1-0.json' ,'theia-e3-1-1.json' ,'theia-e3-1-2.json' ,'theia-e3-1-3.json' ,'theia-e3-1-4.json' ,
             'theia-e3-1-5.json' ,'theia-e3-1-6.json' ,'theia-e3-1-7.json' ,'theia-e3-1-8.json' ,'theia-e3-1-9.json']
